  ...
  return (
    <div className={className}>
      <button
        ...
        className="flex min-h-11 w-full cursor-pointer items-center justify-between gap-4 rounded-sm text-left text-sm font-semibold text-[#EAD9BC] outline-none transition-colors hover:text-[#F7EAD2] focus-visible:ring-3 focus-visible:ring-[#F2C16E]"
        onClick={() => setIsOpen((current) => !current)}
      >
        <span>{label}</span>
        <span
          aria-hidden="true"
          className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full border text-[#E0B15C] transition-[rotate,border-color,background-color] duration-[200ms] ease-[cubic-bezier(0.45,0,0.55,1)] will-change-[rotate] motion-reduce:duration-0 ${
            isOpen
              ? "rotate-45 border-[#E0B15C]/50 bg-[#E0B15C]/8"
              : "rotate-0 border-[#E0B15C]/25"
          }`}
        >
...
export default function VibeSelectionCard({
  ...
  const groupedCues = moodCues.reduce<
    Partial<Record<MoodCue["category"], MoodCue[]>>
  >((groups, cue) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-2xl">
          ...
          <h2
            ...
            className="app-heading-serif mt-1 text-3xl leading-none text-[#F7EAD2] sm:text-4xl"
          >
            What should tonight feel like?
          </h2>
          <p className="mt-3 text-sm leading-6 text-[#EAD9BC] sm:text-base">
            Pick the feeling first. Genres and runtime can stay in the
            background.
          </p>
        </div>
      ...
      <div className="mt-7" aria-busy={moodCuesLoading}>
        {moodCuesLoading ? (
          <p className="text-sm text-[#D9C7A8]" role="status">
            Opening tonight’s choices…
          </p>
        ) : (
          <div>
            ...
            <AnimatedDisclosure
              label="More feelings and occasions"
              className="mt-5 border-t border-[#E0B15C]/10 pt-3"
              contentClassName="grid gap-x-8 gap-y-7 pb-2 pt-4 md:grid-cols-2"
            >
      ...
      <div className="mt-3 border-t border-[#E0B15C]/10 pt-6">
        <AppTextArea
          ...
          classes={{
            ...
            inputWrapper:
              "min-h-[5rem] border-[#E0B15C]/24 !bg-[#22130F]/70 hover:border-[#E0B15C]/40 focus:border-[#E0B15C] focus:ring-1 focus:ring-[#E0B15C]/60 focus:!bg-[#22130F]/70",
          }}
        />
        <p className="mt-1 text-right text-xs text-[#CDB58E]">
          {customMoodText.length} / 240
        </p>
      </div>

      <AnimatedDisclosure
        label="Genre and runtime preferences"
        className="mt-5 border-t border-[#E0B15C]/10 pt-5"
        contentClassName="pb-2 pt-4"
      >
      ...
      <div className="mt-5 flex flex-col gap-4 border-t border-[#E0B15C]/10 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-[#DAC49F]" role="status" aria-live="polite">
          {selectionSummary}
        </p>
        <Button
          size="lg"
